...
function redzlib:MakeWindow(Configs)
    ...
    -- 创建设置容器
    local SettingsContainer = Create("ScrollingFrame", Containers, {
    ...
    -- 添加设置标签
    local SettingsTab = {}
    function SettingsTab:AddSection(Configs)
        return Window:MakeTab():AddSection(Configs)
    end
    function SettingsTab:AddToggle(Configs)
        Configs.__force_container = SettingsContainer
        return Window:MakeTab():AddToggle(Configs)
    end
    function SettingsTab:AddSlider(Configs)
        Configs.__force_container = SettingsContainer
        return Window:MakeTab():AddSlider(Configs)
    end
    function SettingsTab:AddDropdown(Configs)
        Configs.__force_container = SettingsContainer
        return Window:MakeTab():AddDropdown(Configs)
    end
    ...
    S:AddDropdown({
        ...
        Callback = function(Value)
            ...
            if gradientColors[Value] then
                BlueRedGradient.Color = gradientColors[Value][1]
            end
        end
    })
    ...
    S:AddDropdown({
        ...
        Callback = function(Value)
            local themeColors = {
                ["默认蓝色"] = {Color3.fromRGB(0, 180, 240)},
                ["深色模式"] = {Color3.fromRGB(40, 40, 40)},
                ["红色主题"] = {Color3.fromRGB(220, 20, 60)},
                ["绿色主题"] = {Color3.fromRGB(50, 205, 50)},
                ["紫色主题"] = {Color3.fromRGB(147, 112, 219)},
                ["橙色主题"] = {Color3.fromRGB(255, 140, 0)},
                ["粉色主题"] = {Color3.fromRGB(255, 182, 193)},
                ["金色主题"] = {Color3.fromRGB(255, 215, 0)},
                ["青色主题"] = {Color3.fromRGB(0, 255, 255)},
                ["霓虹蓝"] = {Color3.fromRGB(0, 191, 255)},
                ["日落色"] = {Color3.fromRGB(255, 69, 0)},
                ["海洋色"] = {Color3.fromRGB(0, 105, 148)},
                ["玫瑰金"] = {Color3.fromRGB(183, 110, 121)},
                ["矩阵绿"] = {Color3.fromRGB(0, 255, 0)},
                ["深紫色"] = {Color3.fromRGB(75, 0, 130)},
                ["浅色模式"] = {Color3.fromRGB(240, 240, 240)},
                ["暗黑模式"] = {Color3.fromRGB(20, 20, 20)},
                ["炫彩模式"] = {Color3.fromRGB(255, 0, 0)}
            }
            
            if themeColors[Value] then
                Theme["Color Theme"] = themeColors[Value][1]
                redzlib:SetTheme("BlueRed") -- 触发主题更新
            end
        end
    })
    ...
    S:AddDropdown({
        Title = "字体选择",
        Options = {"Gotham", "SourceSans", "Arial", "SciFi", "Code", "Cartoon", "Elegant"},
        Default = "Gotham",
        Callback = function(Value)
            local fontMap = {
                ...
                Elegant = Enum.Font.GothamBold
            }
            
            local selectedFont = fontMap[Value] or Enum.Font.Gotham
            ...
            for _, instance in pairs(redzlib.Instances) do
                if instance.Instance:IsA("TextLabel") or instance.Instance:IsA("TextButton") or instance.Instance:IsA("TextBox") then
                    instance.Instance.Font = selectedFont
                end
            end
        end
    })
    ...
    S:AddSlider({
        Title = "边框粗细",
        Description = "调整UI边框的粗细",
        Default = 2,
        Min = 1,
        Max = 10,
        Callback = function(Value)
            BlueRedStroke.Thickness = Value
        end
    })
    ...
    S:AddSlider({
        ...
        Callback = function(Value)
            ...
            Containers.Size = UDim2.new(1, -Value, 1, -TopBar.Size.Y.Offset)
            SaveJson("redz library V5.json", redzlib.Save)
        end
    })
    ...
    S:AddSlider({
        Title = "文本大小",
        Description = "调整UI中文本的大小",
        Default = 12,
        Min = 8,
        Max = 20,
        Callback = function(Value)
            for _, instance in pairs(redzlib.Instances) do
                if instance.Instance:IsA("TextLabel") then
                    instance.Instance.TextSize = Value
                elseif instance.Instance:IsA("TextButton") then
                    instance.Instance.TextSize = Value
                end
            end
        end
    ...
    S:AddSlider({
        Title = "UI圆角大小",
        Description = "调整UI元素的圆角程度",
        Default = 6,
        Min = 0,
        Max = 20,
        Callback = function(Value)
            for _, instance in pairs(redzlib.Instances) do
                if instance.Instance:IsA("UICorner") then
                    instance.Instance.CornerRadius = UDim.new(0, Value)
                end
            end
        end
    })
    ...
    SettingsButton.Activated:Connect(function()
        ...
        if isSettingsOpen then
            ...
            for _, container in pairs(Containers:GetChildren()) do
                if container:IsA("ScrollingFrame") and container ~= SettingsContainer then
            ...
            -- 高亮设置按钮
            CreateTween({SettingsButton, "ImageColor3", Theme["Color Theme"], 0.2})
        else
            -- 隐藏设置容器
            CreateTween({SettingsContainer, "Size", UDim2.new(1, 0, 1, 150), 0.3, true})
            SettingsContainer.Visible = false
            
            -- 恢复设置按钮颜色
            CreateTween({SettingsButton, "ImageColor3", Color3.fromRGB(255, 255, 255), 0.2})
        end
    end)
