...
if isfile and isfile(DropdownsFile) then
...
-- 加载保存的设置
local saved = {}
if isfile and isfile("sp library.json") then
...
function splib:MakeWindow(Configs)
    ...
    -- 设置 UI 背景图片
    if Configs.BackgroundImage then
        local BackgroundImage = Create("ImageLabel", MainFrame, {
            Image = Configs.BackgroundImage,
            Size = UDim2.new(1, 0, 1, 0),
            BackgroundTransparency = 1,
            ScaleType = Enum.ScaleType.Crop
        })
        Make("Corner", BackgroundImage, UDim.new(0, 10))
    end
    ...
    function Window:MakeTab(paste, Configs)
        ...
        local function SaveBindFlag(flag, keyName)
            local data = {}
            if isfile and isfile(BindsFile) then
                local ok, existing = pcall(function()
        ...
        local function SaveSliderFlag(flag, value)
            local data = {}
            if isfile and isfile(SlidersFile) then
                local ok, existing = pcall(function()
        ...
        local function SaveTextBoxFlag(flag, value)
            local data = {}
            if isfile and isfile(TextBoxesFile) then
                local ok, existing = pcall(function()
        ...
        local function SaveColorPickerFlag(flag, color: Color3)
            local data = {}
            if isfile and isfile(ColorPickersFile) then
                local ok, existing = pcall(function()
        ...
        -- 字体设置
        local availableFonts = {"Gotham", "GothamMedium", "GothamBold", "SourceSans", "SourceSansBold", "FredokaOne"}
        ...
        Tab:AddDropdown({
            ...
            Callback = function(selectedFont)
                ...
                for _, instanceData in pairs(splib.Instances) do
                    if instanceData.Instance:IsA("TextLabel") or instanceData.Instance:IsA("TextBox") or instanceData.Instance:IsA("TextButton") then
